#include <RCSwitch.h> // Librairie pour utiliser la radiofréquence de 433 MHz 
#include <EEPROM.h> // Librairie pour utiliser les fonctionnalités de l EEPROM
...
unsigned long time_previousCommand, latence(200); // en milliseconds

// creation d une methode et déclaration d une variable data
RCSwitch myRadio = RCSwitch();
unsigned long data = 0;   // stockage des informations recu
...
bool inCorrectionOE(false), inCorrectionNS(false); // variable pour definir un etat de correction lors d'une correction

int Speed(98); // vitesse moteur
int TrSN(55); // = S-N
int TrOE(-5); // = O-E
int val_min_soleil(0); // seuil de tolerance
int average; // moyenne nord et ouest

//TrSN and TrOE need to be upgraded automatically
// creation d une tolerance
int ErrorSN(15); // = average * un coef
int ErrorOE(25); // = average * un coef

void setup() {

  DDRC = 0xFF; // configuration du port C comme un input port

  //Initiate PWM on Timer 2
  TCCR2A = 0b10100011; // fast pwm mode
  TCCR2B = 0b00000001; // prescaler de 2
  OCR2A = 0; //D11 duty cycle pin 11
  OCR2B = 0; //D3 duty cycle pin 11
  ...
  // Configuration des pins pour les cellules photovoltaiques
  pinMode(NORD, INPUT);
  pinMode(SUD, INPUT);
  pinMode(OUEST, INPUT);
  pinMode(EST, INPUT);
  ...
  // Configuration des pins pour les fins de courses
  ...
  myRadio.enableReceive(0); // Receiver on interrupt at digital pin #2 ( le 0 correspond au pin 2 )
}

void loop()
{
  // variable locale pour gestion du temps
  unsigned long debut = millis(); // heure d'activation
  unsigned long atm = millis(); // heure actuelle (at this moment)
  ...
  if (myRadio.available()) // si la telecommande envoie une donnee
  {
    ...
    time_previousCommand = millis() + latence; // heure de la dernniere commande recu

    switch (data) // Selon le button qu on a presse
    {
      case 192: // bouton 1 on monte la surface, 3 pour la telecommande a 12 boutons
        if (digitalRead(END_HAUT) == HIGH) // check si on est en butee
        {
          debut = millis(); // heure de demarrage
          atm = millis(); // heure actuelle

          // tant qu on n est pas en butee et que le temps n est pas ecoule
          while (( atm - debut < 100 ) && (digitalRead(END_HAUT) == HIGH))
          {
            engine_control(true, true, 98);
            atm = millis(); // on releve le temps
          }
          automatic_mode = false; // on sort le systeme du mode auto s il etait dans cette etat
          break;
        }
      ...
      case 48: // bouton 2 on descend la surface, 12 pour la telecommande a 12 boutons

        if (digitalRead(END_BAS) == HIGH) // check si on est en butee
        {
          debut = millis(); // heure de demarrage
          atm = millis(); // heure actuelle
          while (( atm - debut < 100 ) && (digitalRead(END_BAS) == HIGH)) // tant qu on n est pas en butee et que le temps n est pas ecoule
          {
      ...
      case 240: // bouton 3 on tourne vers la gauche, 15 pour la telecommande a 12 boutons

        if (digitalRead(END_DROITE) == HIGH) // check si on est en butee
        {
          debut = millis(); // heure de demarrage
          atm = millis(); // heure actuelle
          while (( atm - debut < 100 ) && (digitalRead(END_DROITE) == HIGH)) // tant qu on n est pas en butee et que le temps n est pas ecoule
          {
      ...
      case 12: // bouton 4 on tourne vers la droite, 48 pour la telecommande a 12 boutons
        if (digitalRead(END_GAUCHE) == HIGH) // check si on est en buté
        {
          debut = millis(); // heur de demarrage
          atm = millis(); // heur actuelle
          while (( atm - debut < 100 ) && (digitalRead(END_GAUCHE) == HIGH)) // tant qu on n est pas en butee et que le temps n est pas ecoule
          {
      ...
      case 204: // bouton 5, 51 pour la telecommande a 12 boutons
        automatic_mode = false;
        stop_engine(); // arret des moteurs
        break;

      case 60: // bouton 6, 60 pour la telecommande a 12 boutons
        automatic_mode = false;
        stop_engine(); // arret des moteurs
        break;

      case 252: // bouton 7 releve de l etat des cellules et suivi soleil, 63 pour la telecommande a 12 boutons
      ...
      case 3: // bouton 8 recherche du soleil, centrage du capteur, releve des cellules, suivi soleil 192 pour la telecommande a 12 boutons
        scrutationRapide(); // recherche du soleil
        scrutationLente(); // centrage des cellules
        Calibration(); // releve des valeures des cellules et sauvegarde
        asservissement(); // tracking du soleil
        automatic_mode = true;
        break;
  ...
  else // Si aucun info de la part de la telecomande
  {
    ...
    // ajouter soleil
    if (automatic_mode && (average > val_min_soleil) && (analogRead(soleil) > 5) )
    {
      time_previousCommand = millis() + latence;
      asservissement(); // tracking du soleil
    }
    // si ca fait un moment qu il n y a pas eu de correction on coupe les moteurs
    else if (millis() > time_previousCommand && !alreadyStop)
    {
      alreadyStop = true;
      stop_engine(); // arret des moteurs
      OCR2A = 0;
    ...
    // seuil a modifier
    else if (analogRead(soleil) < 5) // si le soleil disparait
    {
      stop_engine();
      debut = millis(); // heur de demarrage
      atm = millis(); // heur actuelle
      ...
      // duree a modifier
      while (atm - debut < 120000) // si on perd la lumiere cela peut etre un nuage donc on attend et interoge soleil tout en ecoutant la telecommande
      {
        atm = millis(); // on releve le temps

        if ( (checkRemote() == true) || (analogRead(soleil) > 5) ) // on verifie la telecommande et la lumiere
        {
          return;
        }
      }
      ...
      // seuil a modifier
      if (analogRead(soleil) < 5) // on considere qu on est en nuit
      {
        automatic_mode = false;

        // le systeme se place en debut de scrutation rapide pour le jour suivant
        engine_control(false, true, 98);
        while (digitalRead(END_GAUCHE) == HIGH)
        {
          if ( checkRemote() == true ) // on verifie la telecommande et la lumiere
          {
            return;
          }
        }
        ...
        engine_control(true, false, 98);
        while (digitalRead(END_BAS) == HIGH)
        {
          if ( (checkRemote() == true) || (analogRead(soleil) < 5) ) // on verifie la telecommande et la lumiere
          {
            return;
        ...
        while (analogRead(soleil) < 5) // on rentre dans la boucle tant qu on est en nuit mais on ecoute la telecommande
        {
          if ( checkRemote() == true ) // on verifie la telecommande
          {
            return;
          }
        }

        // en sortie de nuit on refais une scrutation
        scrutationRapide(); // recherche du soleil
        scrutationLente(); // centrage des cellules
        Calibration(); // releve des valeures des cellules et sauvegarde
        asservissement(); // tracking du soleil
...
{ // 50 au projo
  // 150 ext
  if ((analogRead(NORD) > 150) && (analogRead(EST) > 150)) // si nord et est son > a un seuil
  {
    return true;
  }
  else
...
bool checkRemote() // regarde si un ordre est envoye via la telecommande
{
  data = 0;
  data = myRadio.getReceivedValue() & 255; // stockage de la donne et conversion
  // if ( (data == 3) || (data == 12) || (data == 15) || (data == 48) || (data == 51) || (data == 60)  || (data == 63) || (data == 192)) // pour telecommande 12 boutons, ne pas mettre le mode auto et le suivi
  if ( (data == 60) || (data == 192) || (data == 48) || (data == 240) || (data == 12) || (data == 204) ) // touches de la telecommande
  {
...
// 1 cycle de scrutationRapide prend 22min pour faire de bas gauche a haut droite
void scrutationRapide() // cherche ou se trouve le soleil
{
  // variable locale pour gestion du temps
  unsigned long debut = millis(); // heur d'activation
  unsigned long atm = millis(); // heur actuelle (at this moment)

  if (analogRead(soleil) > 5) // on verifie la lumiere
  {
    ...
    { // au debut de scrutationRapide nous allons en bas a gauche qui est une position connu pour aller en haut a droite
      engine_control(false, true, 98);
      while (digitalRead(END_GAUCHE) == HIGH)
      {
        if (checkRemote() == true ) // on verifie la telecommande
      ...
      while (((digitalRead(END_HAUT) == HIGH) && (digitalRead(END_DROITE) == HIGH))) // tant que les FC haut et droite ne sont pas atteint et que le capteur n est pas trouve
      {
        ...
        while (digitalRead(END_DROITE) == HIGH) // tant qu on n est pas en butee
        {
          ...
          else if (checkRemote() == true ) // on verifie la telecommande
          {
            return;
          }
        }
        ...
        if (digitalRead(END_HAUT) == HIGH) // si je ne suis pas en butee haute
        {
          debut = millis(); // heur de demarrage
          atm = millis(); // heur actuelle
          while ( (atm - debut < 5000) && (digitalRead(END_HAUT) == HIGH) ) // tant qu on n est pas en butee haute et qu on a pas depasse 5 sec
          {
            if (checkRemote() == true ) // on verifie la telecommande
            {
              return;
        ...
        if (digitalRead(END_HAUT) == HIGH) // si je ne suis pas en butee haute
        {
          ...
          while (digitalRead(END_GAUCHE) == HIGH) // tant que je ne suis pas en butee gauche
          {
            if (isSensorFound() == true) // si le capteur est trouve on sort
            {
              return;
    ...
    reverseScrutationRapide(); // si capteur pas trouvé fait le chemin en sens inverse
  }
  else
  {
    return;
...
void reverseScrutationRapide() // cherche ou se trouve le soleil dans le sens oppose a scrutationRapide
{
  // variable locale pour gestion du temps
  unsigned long debut = millis(); // heur d'activation
  unsigned long atm = millis(); // heur actuelle (at this moment)
  ...
  if (analogRead(soleil) > 5) // on verifie la lumiere
  {
    ...
    else
    {
      while ( ( (digitalRead(END_BAS) == HIGH) && (digitalRead(END_GAUCHE) == HIGH) ) ) // tant que les FC bas et gauche ne sont pas atteint et que le capteur n est pas trouve
      {
        ...
        while (digitalRead(END_GAUCHE) == HIGH) // tant qu on n est pas en butee gauche
        {
          if (isSensorFound() == true) // si le capteur est trouve on sort
          {
            return;
          }
          else if ( (checkRemote() == true) || (analogRead(soleil) > 5) ) // on verifie la telecommande et la lumiere
          {
            return;
          }
        }
        ...
        if (digitalRead(END_BAS) == HIGH) // si je ne suis pas en butee basse
        {
          debut = millis(); // heur de demarrage
          atm = millis(); // heur actuelle
          while ( (atm - debut < 5000) && (digitalRead(END_BAS) == HIGH) ) // tant qu on n est pas en butee basse et qu on n a pas depasse 5 sec
          {
            if ( (checkRemote() == true) || (analogRead(soleil) > 5) ) // on verifie la telecommande et la lumiere
            {
              return;
        ...
        stop_engine();
        if (digitalRead(END_BAS) == HIGH) // si je ne suis pas en butee haute
        {
          engine_control(false, false, 98); // go gauche
          while (digitalRead(END_DROITE) == HIGH) // tant que je ne suis pas en butee gauche
          {
            if (isSensorFound() == true) // si le capteur est trouve on sort
            {
              return;
    ...
    scrutationRapide(); // si le capteur n est pas trouve, fait le chemin en sens inverse
  }
  else
  {
    return;
...
void scrutationLente() // centre la lumière refletee par rapport au cellules photovoltaiques
{
  int valOuest, valEst, valNord, valSud; // variable lecture des cellules
  ...
  if (analogRead(soleil) > 5) // on verifie la lumiere
  {
    if  ( ( (abs(valNord - valSud) < 0) || (abs(valNord - valSud) > 10) ) ||  ( (abs(valEst - valOuest) < 0) || (abs(valEst - valOuest) > 10) ) ) // Si on n est pas centre
    {
      while ( ( (valNord - valSud < 0) || (valNord - valSud > 5) ) && ( (valEst - valOuest < 0) || (valEst - valOuest > 5) ) ) // tant qu on n est pas centre
      {
        if ( ( (valEst - valOuest) > 0) && (digitalRead(END_GAUCHE) == HIGH) ) // si E>O et on n est pas en butee haute
        {
        ...
        if ( (valOuest - valEst > 0) && (digitalRead(END_DROITE) == HIGH) ) // si O>E et on n est pas deja en butee
        {
        ...
        if ((valNord - valSud > 0) && (digitalRead(END_BAS) == HIGH)) // si N>S et on n est pas deja en butee
        {
        ...
        if ((valSud - valNord > 0) && (digitalRead(END_HAUT) == HIGH) ) // si S>N et on n est pas deja en butee
        {
...
void Calibration() // save les valeurs des capteurs afin d'avoir une reference
{
  ...
  ErrorOE = average * 0.02; // coeficient de sensibilite OUEST EST
  ErrorSN = average * 0.02; // coeficient de sensibilite NORD SUD
