...
// regarde si moyenne des cellules nord ouest est >410
// moi je vais faire une moyen des 4 
bool isSensorFound()
{
  int Nord = analogRead(NORD);
  int Ouest = analogRead(OUEST);
  average = (analogRead(NORD) + analogRead(OUEST))/0,5;
  if (average > val_min_soleil)
  {return true;}
  return false;
}

//Check if it's the night or the day. Use the luminosity sensor
// 41 correspond a 0,02V
bool isNight()
{
  int currentVal = analogRead(soleil);
  if (currentVal < 41)
  {
    return true;
  } else {
...
// démarre d'une pos random pour go haut droite
void fastCalibration()
{
  while(((digitalRead(END_HAUT)==HIGH)&&(digitalRead(END_DROITE)==HIGH)) ) // tant que FC haut et droite sont pas atteint et que le capteur n'est pas trouvé
  {
    ...
    while(digitalRead(END_DROITE)==HIGH){} // tant que FC droite ouvert
    stop_engine();
    ...
    if(digitalRead(END_HAUT)==HIGH) // si je suis pas en buté haute
    {
      ...
      while(digitalRead(END_GAUCHE)==HIGH){} // tant que FC gauche ouvert
      stop_engine();      
    }
...
bool isThereSun() // check si la 5eme cellule a une valeur > 410 correspond a 0,2V
{
  int sunCaptor = analogRead(soleil);
  if (sunCaptor > val_min_lum)
    return true;
  else
    return false;
...
/*bool isLimitReached()
{
  int Fin_Course_Gauche = 0;
  int Fin_Course_Droite = 0;

  Fin_Course_Gauche = digitalRead(END_GAUCHE);
  Fin_Course_Droite = digitalRead(END_DROITE);

  if ((Fin_Course_Gauche == LOW) || (Fin_Course_Droite == LOW))
  {
    return true;
  }
  else
...
bool isLimitReachedHB()
{
  int Fin_Course_Haut = 0;
  int Fin_Course_Bas = 0;

  Fin_Course_Haut = digitalRead(END_HAUT);
  Fin_Course_Bas = digitalRead(END_BAS);

  if ((Fin_Course_Bas == LOW) || (Fin_Course_Haut == LOW))
  {
    return true;
  }
  else
